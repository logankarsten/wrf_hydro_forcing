...
import Analysis_Assimilation_Forcing as anal_assim
import datetime
import re
...
def is_within_time_range(start_dt, end_dt, file, prod, is_yellowstone=False):
    ...
    if is_yellowstone:
        ...
        if prod == 'RAP' or prod == 'HRRR':
            match = re.match(r'.*/([0-9]{8})/[0-9]{8}_i[0-9]{2}_f[0-9]{2,3}_(WRF-RR|HRRR).*',file)
            ymd_dir = match.group(1)
        elif prod == 'MRMS':
            match = re.match(r'.*/GaugeCorr_QPE_00.00_([0-9]{8})_[0-9]{6}.grib2',file)
            ymd_dir = match.group(1)
         
        else:
            print("Requested data product is unrecognized or not supported, please try again")
            return False
...
def main():
    """Tests the regridding and downscaling of RAP and HRRR
       data for the Analysis and Assimilation Forcing Configuration.
    """
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    # MRMD_dir_base = "/glade/scratch/lpan/IOC/data/MRMS"
...
def do_regrid(dir_base, prod, data_files, is_yellowstone):
    ...
    for file in data_files:
        ...
        if prod == 'RAP' or prod == 'HRRR':
            ...
            anal_assim.forcing("regrid",prod,file_only)
        elif prod == 'MRMS':
            match = re.match(r'.*/[0-9]{8}/(GaugeCorr_QPE_00.00_[0-9]{8}_[0-9]{6}.grib2)',file)
...
def do_layering(rap_downscale_dir, hrrr_downscale_dir, is_yellowstone=False):
    ...
    for file in files_to_layer:
        anal_assim.forcing("layer","RAP", file, "HRRR", file)
